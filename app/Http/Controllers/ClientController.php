...
class ClientController extends Controller
{
    ...
    public function create(Request $request)
    {
        ...
        if(\Auth::user()->can('create client'))
        {
            ...
            else
            {
                ...
                $user = \Auth::user();
                $travelAgencies = Customer::where('created_by', '=', $user->creatorId())
                    ->where('tax_number', '=', 'Travel Agency')
                    ->orderBy('name', 'asc')
                    ->pluck('name', 'id');

                $branches = Branch::where('created_by', '=', $user->creatorId())
                    ->orderBy('name', 'asc')
                    ->pluck('name', 'id');

                return view('clients.create', compact('customFields', 'travelAgencies', 'branches'));
            }
        }
        else
        {
    ...
    public function store(Request $request)
    {
        ...
        if(\Auth::user()->can('create client'))
        {
            ...
            // Determine validation rules based on whether it's an ajax request
            if($request->ajax)
            {
            ...
            // Get customer name from customer_id or use name directly for ajax requests
            if($request->ajax)
            {
    ...
    public function edit(User $client)
    {
        if(\Auth::user()->can('edit client'))
        {
            ...
            if($client->created_by == $user->creatorId())
            {
                ...
                return view('clients.edit', compact('client', 'customFields', 'branches'));
            }
            else
            {
        ...
        if(\Auth::user()->can('edit client'))
        {
            ...
            if($client->created_by == $user->creatorId())
            {
                ...
                if(!empty($request->password_switch) && $request->password_switch == 'on')
                {
                    ...
                    {
                        // If password switch is on but no password provided, check if client already has password
                ...
                $post['is_enable_login'] = $enableLogin;
