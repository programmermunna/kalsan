                    ...
                    r ? dragula(n, {
                        ...
                        $.ajax({
                            ...
                            success: function(data) {
                                show_toastr('success', "Bug Moved Successfully.",
                                    'success');
                            },
                            error: function(data) {
                ...
                $.ajax({
                    ...
                    success: function(data) {
                        var html = "<li class='media mb-20'>" +
                            ...
                            "                        <p class='mb-0 text-xs'>" + data.data.comment +
                            "</p></div>" +
                            "                           <div class='comment-trash' style=\"float: right\">" +
                            "                               <a href='#' class='btn btn-sm red btn-danger delete-comment' data-url='" +
                            data.data.deleteUrl + "' >" +
            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    if (data.deleteUrl.length > 0) {
                        delLink =
                            "<a href='#' class='text-danger text-muted delete-comment-file'  data-url='" +
                            data.deleteUrl + "'>" +
                    ...
                    var html = '<div class="col-8 mb-2 file-' + data.id + '">' +
                        ...
                        '">' +
                        '                                    <div class="comment-trash" style="float: right">' +
                        '                                        <a download href="{{ asset(Storage::url('bugs')) }}/' +
                        data.file + '" class="btn btn-sm btn-primary me-1">' +
                        '                                            <i class="ti ti-download"></i>' +
                        '                                        </a>' +
                        '                                        <a href="#" class="btn btn-sm red btn-danger delete-comment-file m-0 px-2" data-id="' +
...
@section('action-btn')
    <div class="float-end">
        @can('manage bug report')
            <a href="{{ route('task.bug', $project->id) }}" data-bs-toggle="tooltip" title="{{ __('List') }}"
                class="btn btn-sm btn-primary-subtle me-1">
                <i class="ti ti-list"></i>
            </a>
        @endcan
        @can('create bug report')
            <a href="#" data-size="lg" data-url="{{ route('task.bug.create', $project->id) }}" data-ajax-popup="true"
                data-bs-toggle="tooltip" title="{{ __('Create New Bug') }}" class="btn btn-sm btn-primary">
                ...
                @foreach ($bugStatus as $status)
                    ...
                    <div class="col">
                        <div class="crm-sales-card mb-4">
                            ...
                            <div class="sales-item-wrp kanban-box" id="task-list-{{ $status->id }}"
                                data-id="{{ $status->id }}">
                                @foreach ($bugs as $bug)
                                    <div class="sales-item draggable-item" id="{{ $bug->id }}">
                                        <div class="sales-item-top border-bottom">
                                            <div class="d-flex align-items-center">
                                                <h5 class="mb-0 flex-1">
                                                    <a href="#" class="dashboard-link"
                                                        data-url="{{ route('task.bug.show', [$project->id, $bug->id]) }}"
                                                        data-ajax-popup="true" data-size="lg"
                                                        data-bs-original-title="{{ $bug->title }}">{{ $bug->title }}</a>
                                                </h5>
                                                    ...
                                                    @if (Gate::check('edit bug report') || Gate::check('delete bug report'))
                                                        <div class="dropdown-menu icon-dropdown dropdown-menu-end">
                                                            @can('edit project task')
                                                                <a href="#!" data-size="lg"
                                                                    data-url="{{ route('task.bug.edit', [$project->id, $bug->id]) }}"
                                                                    data-ajax-popup="true" class="dropdown-item"
                                                                    data-bs-original-title="{{ __('Edit ') . $bug->name }}">
                                            ...
                                            <div class="badge-wrp d-flex flex-wrap align-items-center gap-2">
                                                @if ($bug->priority == 'low')
                                                    <span
                                                        class="badge p-2 rounded text-md f-w-600 bg-light-success">{{ ucfirst($bug->priority) }}</span>
                                                @elseif($bug->priority == 'medium')
                                                    <span
                                                        class="badge p-2 rounded text-md f-w-600 bg-light-warning">{{ ucfirst($bug->priority) }}</span>
                                                @elseif($bug->priority == 'high')
                                                    <span
                                                        class="badge p-2 rounded text-md f-w-600 bg-light-danger">{{ ucfirst($bug->priority) }}</span>
                                                @endif
                                            </div>
                                        </div>
                                        <div
                                            class="sales-item-center border-bottom d-flex align-items-center justify-content-between">
                                            <span data-bs-toggle="tooltip" title="{{ __('Start Date') }}">
                                        ...
                                        <div class="sales-item-bottom d-flex align-items-center justify-content-between">
                                            <div class="user-group">
                                                <img @if (isset($user[0]->avatar)) src="{{ asset('/storage/uploads/avatar/' . $user[0]->avatar) }}" @else src="{{ asset('/storage/uploads/avatar/avatar.png') }}" @endif
                                                    alt="image" data-bs-toggle="tooltip"
                                                    title="{{ !empty($user[0]) ? $user[0]->name : '' }}">
                                            </div>
                                        </div>
                                    </div>
                                @endforeach
