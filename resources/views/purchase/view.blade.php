...
@section('content')
                            ...
                            <div class="row timeline-wrapper">
                                <div class="col-xl-4 col-md-5 col-sm-7 create-invoice invoice">
                                    ...
                                    <div class="bill-timeline-inner d-flex gap-3">
                                        ...
                                        <div class="timeline-content text-start">
                                            <h5 class="mb-2">{{ __('Create Cargo') }}</h5>
                                            <p class="text-muted mb-2">
                                                {{ __('Created on ') }}{{ \Auth::user()->dateFormat($purchase->purchase_date) }}</p>
                                            @can('edit purchase')
                                ...
                                <div class="col-xl-4 col-md-5 col-sm-7 send-invoice invoice">
                                    ...
                                    <div class="bill-timeline-inner d-flex gap-3">
                                        ...
                                        <div class="timeline-content text-start">
                                            <h5 class="mb-2">{{ __('Approve Cargo') }}</h5>
                                            <p class="text-muted mb-2">
                                                ...
                                                @can('send purchase')
                                                    <a href="{{ route('purchase.sent', $purchase->id) }}"
                                                        ...
                                                        data-bs-toggle="tooltip" data-original-title="{{ __('Mark Sent') }}"><i
                                                            class="ti ti-send mr-2"></i>{{ __('Approve') }}</a>
                                                @endcan
                                            @endif
                                        </div>
                                    </div>
                                        ...
                                        @if (!empty($vendor))
                                            <div class="col">
                                                <small class="font-style">
                                                    ...
                                                    @endif
                                                    @if (!empty($vendor->billing_city))
                                                        {{ $vendor->billing_city }}
                                                        @if (!empty($vendor->billing_state))
                                                            , {{ $vendor->billing_state }}
                                                        @endif
                                                        <br>
                                                    @endif
                                                    @if (!empty($vendor->billing_zip))
                                                        {{ $vendor->billing_zip }}<br>
                                                    @endif
                                                    @if (!empty($vendor->billing_country))
                                                        {{ $vendor->billing_country }}<br>
                                                    @endif
                                                    @if ($settings['vat_gst_number_switch'] == 'on' && !empty($vendor->tax_number))
                                                        <strong>{{ __('Tax Number') }} :</strong> {{ $vendor->tax_number }}
                                                    @endif
                                                </small>
                                            </div>
                                        @endif
            ...
            2,
                                    ...
                                    <div class="row mt-3">
                                        <div class="col-md-6">
                                            <small>
                                                <strong>{{ __('Origin') }} :</strong><br>
                                                {{ !empty($purchase->origin) ? $purchase->origin : '-' }}
                                            </small>
                                        </div>
                                        <div class="col-md-6">
                                            <small>
                                                <strong>{{ __('Destination') }} :</strong><br>
                                                {{ !empty($purchase->destination) ? $purchase->destination : '-' }}
                                            </small>
                                        </div>
                                    </div>
                                    <div class="row mt-3">
                                            ...
                                            <div class="table-responsive mt-3">
                                                <table class="table ">
                                                    <tr>
                                                        ...
                                                        <th class="text-dark">{{ __('Cargo') }}</th>
                                                        <th class="text-dark">{{ __('Description') }}</th>
                                                        <th class="text-dark">{{ __('Gross Weight') }}</th>
                                                        <th class="text-dark">{{ __('Rate') }}</th>
                                                        <th class="text-dark">{{ __('Other Frieght') }}</th>
                                                        <th class="text-dark">{{ __('Agent Fee') }}</th>
    ...
    $taxesData = [];
                                                    ...
                                                    @foreach ($iteams as $key => $iteam)
                                                        <tr>
                                                            ...
                                                            <td>{{ \Auth::user()->priceFormat($iteam->other_frieght) }}</td>
                                                            <td>{{ \Auth::user()->priceFormat($iteam->agent_fee) }}</td>
                                                            <td>{{ \Auth::user()->priceFormat($iteam->discount) }}</td>
                                                            @php
        $totalQuantity += $iteam->quantity;
        $subTotal += ($iteam->quantity * $iteam->price);
        $totalDiscount += $iteam->discount;
        $totalOtherFrieght += $iteam->other_frieght;
        $totalAgentFee += $iteam->agent_fee;
        $itemAmount = ($iteam->quantity * $iteam->price) + $iteam->other_frieght + $iteam->agent_fee - $iteam->discount;
        $totalAmount += $itemAmount;
                                                            ...
                                                            <td class="text-end">
                                                                @php
                                                                    $itemAmount = ($iteam->quantity * $iteam->price) + $iteam->other_frieght + $iteam->agent_fee - $iteam->discount;
                                                                @endphp
                                                                {{ \Auth::user()->priceFormat($itemAmount) }}
                                                            </td>
                                                        </tr>
                                                    @endforeach
                                                    <tfoot>
                                                        ...
                                                        <tr>
                                                            ...
                                                            <td class="text-end">
                                                                @php
                                                                    $subTotal = ($iteam->quantity * $iteam->price) + $iteam->other_frieght + $iteam->agent_fee;
                                                                @endphp
                                                                {{ \Auth::user()->priceFormat($subTotal) }}</td>
                                                        </tr>
                                                        {{-- <tr>
                                                            <td colspan="7"></td>
                                                            <td class="text-end"><b>{{ __('Other Freight') }}</b></td>
                                                            <td class="text-end">
                                                                {{ \Auth::user()->priceFormat($totalOtherFrieght) }}
                                                            </td>
                                                        </tr>
                                                        <tr>
                                                            <td colspan="7"></td>
                                                            <td class="text-end"><b>{{ __('Agent Fee') }}</b></td>
                                                            <td class="text-end">
                                                                {{ \Auth::user()->priceFormat($totalAgentFee) }}
                                                            </td>
                                                        </tr> --}}
                                                        <tr>
                                                            <td colspan="7"></td>
                                                        ...
                                                        <tr>
                                                            ...
                                                            <td class="blue-text text-end">
                                                                {{ \Auth::user()->priceFormat($totalAmount) }}</td>
                                                        </tr>
                                                        <tr>
                                                            <td colspan="7"></td>
    ...
    foreach ($purchase->payments as $payment) {
        $totalPaid += $payment->amount;
                                                                ...
                                                                @endphp
                                                                {{ \Auth::user()->priceFormat($totalPaid) }}
                                                            </td>
                                                        </tr>
                                                        <tr>
                                                            <td colspan="7"></td>
    ...
    $dueAmount = $totalAmount - $totalPaid;
                                                                @endphp
                                                                {{ \Auth::user()->priceFormat($dueAmount) }}</td>
                                                        </tr>
                                                    </tfoot>
                                                </table>
                                            </div>
                                            ...
                                            <td>
                                                @if (!empty($payment->add_receipt))
                                                    <a href="{{ asset(Storage::url('uploads/payment')) . '/' . $payment->add_receipt }}"
                                                        ...
                                                        data-bs-original-title="{{ __('Download') }}"><span
